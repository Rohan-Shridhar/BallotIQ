 ...
 * Shows users whether they are getting Live AI, Cached, or Offline responses.
 * 
 * @param mode - Current operational state: 'live', 'cached', or 'error' (inactive)
...
export default function AIStatusBadge({ mode, className = '' }: AIStatusBadgeProps) {
  ...
  return (
    <div 
      className={`inline-flex items-center gap-2 px-3 py-1 rounded-full border text-[10px] font-bold uppercase tracking-widest ${config.color} ${className}`}
      aria-live="polite"
    >
      {config.pulse && (
